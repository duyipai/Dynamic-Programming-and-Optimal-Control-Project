function figH = PlotMap3(fig, mapSize, map, gate, mansion, cameras, varargin)
%PLOTMAP3 Plot a 3D-map and the control action in each cell.
%   
%	figH = PlotMaze3(fig, mapSize, map, gate, mansion, cameras, varargin)
%
%	Input arguments:
%
%       fig:
...
% ground
zStartHeight = -1;
groundColor = [127 156 89]./255;

% tree
treeColorLow = [95 189 64]./255;
treeColorHigh = [96 120 87]./255;
maxtreeHeight = 2;
...
% mansion
mansionColor = [255 255 255]./255;
mansionHeight = 2;
roofColor = [184 77 6]./255;
roofHeight = 0.35;
...
% remove gate from ground
[idxwater1] = findsubmat([column row], gate);
row(idxwater1) = [];
column(idxwater1) = [];
...
for i = 1:length(row)
        xCorner = column(i)-1;
        yCorner = row(i)-1;
        zCorner = zStartHeight;
        voxel([xCorner, yCorner, zCorner], [1 1 -zStartHeight], groundColor);
        hold on;
end
...
for i = 1:length(row)
        ...
        if (isempty(dummyIdx1))
            ...
            yCorner = row(i)-1;
            zCorner = 0;
            treeHeight = min(map(row(i),column(i)), maxtreeHeight);
            treeColor = interp1([0;1], [treeColorLow; treeColorHigh],treeHeight/maxtreeHeight);
            voxel([xCorner, yCorner, zCorner], [1 1 map(row(i),column(i))], treeColor);
            hold on;
        end
end
...
for i = 1:length(row)
        ...
        yCorner = row(i)-1;
        zCorner = zStartHeight;
        voxel([xCorner, yCorner, zCorner], [1 1 1+map(row(i),column(i))], waterColor);
        hold on;
end
...
yCorner = gate(2)-1;
zCorner = zStartHeight;
voxel([xCorner, yCorner, zCorner], [1 1 -zStartHeight], gateColor);
hold on;
...
for i = 1:length(row)
        ...
        if ((row(i) == min(row) || row(i) == max(row)) && (column(i) == min(column) || column(i) == max(column)))
            voxel([xCorner, yCorner, zCorner], [1 1 mansionHeight-cameraHeight], mansionColor);
            hold on;
            zCorner = mansionHeight - cameraHeight;
            voxel([xCorner, yCorner, zCorner], [1 1 cameraHeight], cameraColor);
            hold on;
        else
            voxel([xCorner, yCorner, zCorner], [1 1 mansionHeight], mansionColor);
            hold on;
        end
end

% plot roof
nLowRoof = min(column) + 0.5;
nHighRoof = max(column) - 0.5;
mLowRoof = min(row) + 0.5;
mHighRoof = max(row) - 0.5;

n = nLowRoof:1:nHighRoof;
m = mLowRoof:1:mHighRoof;

roofLevel = 0;
while (~isempty(n) || ~isempty(m))
    for i = 1:length(n)
        for j = 1:length(m)
            xCorner = n(i) - 1;
            yCorner = m(j) - 1;
            zCorner = mansionHeight + roofLevel*roofHeight;
            
            voxel([xCorner, yCorner, zCorner], [1 1 roofHeight], roofColor);
            hold on;
        end
    end
...
if nargin > 8
    ...
    for i = 1 : length( J );
        ...
        if shouldPlotArrow
            ...
            if (~isempty(u_i))
                startPt = center + 0.0 * u_i;
                endPt = center + 0.4 * u_i;
                arrow3( startPt, endPt, arrowHeight );
            else
                circle3( center, 0.3 , arrowHeight );
            end
        end
    end
end
...
daspect([1 1 1]);
margin = 2;
axis([-margin mapSize(1)+margin -margin mapSize(2)+margin zStartHeight 5]);
view([60 30]);

axesHandle = findobj(gcf,'Type','axes');
set(axesHandle,'Color',[0.94 0.94 0.94]);
set(axesHandle,'ZTick',[]);
set(axesHandle,'ZColor',[0.94 0.94 0.94]);
xlabel('x');
ylabel('y');
...
%% plot 2D arrow in 3D plot
function arrow3( startPt, endPt, height )
...
% plot
color = [ 0, 0, 0 ];
plot3( arrowLines( :, 1 ), arrowLines( :, 2 ), height*ones(size(arrowLines(:,1))), 'color', color, 'lineWidth', 1.5 );
plot3( arrowHead( :, 1 ), arrowHead( :, 2 ), height*ones(size(arrowHead(:,1))),'color', color, 'lineWidth', 1.5 );

end

%% plot 2D circle (disk) in 3D plot
function circle3( center, radius, height )
theta=-pi:0.1:pi;
x = center(1) + radius*cos(theta);
y = center(2) + radius*sin(theta);
z = height*ones(size(y));
fill3(x, y, z, [0, 0, 0], 'EdgeColor', 'none');
end
